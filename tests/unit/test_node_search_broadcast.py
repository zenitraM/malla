...
class TestNodeSearchBroadcast:
    ...
    def test_node_search_broadcast_query_variations(self):
        ...
        with app.test_client() as client:
            for query in test_queries:
                with patch(
                    "src.malla.routes.api_routes.get_db_connection"
                ) as mock_conn:
                    ...
                    assert len(data["nodes"]) == 1, (
                        f"Wrong nodes length for query: {query}"
                    )
                    ...
                    broadcast_node = data["nodes"][0]
                    assert broadcast_node["node_id"] == 4294967295, (
                        f"Wrong node_id for query: {query}"
                    )
                    assert broadcast_node["hex_id"] == "!ffffffff", (
                        f"Wrong hex_id for query: {query}"
                    )
    ...
    def test_api_nodes_broadcast_query_variations(self):
        ...
        test_queries = ["broadcast", "ffffffff", "!ffffffff", "4294967295"]
    ...
    def test_node_search_non_broadcast_query(self):
        ...
        test_queries = ["test", "node123", "12345678", "!12345678", "some random text"]
            ...
            for query in test_queries:
                with patch(
                    ...
                    assert data["total_count"] == 0, (
                        f"Expected 0 results for query: {query}"
                    )
                    assert len(data["nodes"]) == 0, (
                        f"Expected empty nodes for query: {query}"
                    )
